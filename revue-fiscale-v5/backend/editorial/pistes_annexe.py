"""Pistes Annexe 2026 × file a_confirmer — outillage éditorial, pas visa."""
...
FICHIER_PISTES = RACINE / "referentiel" / "propositions_annexe_2026_pistes.json"
SOURCE_PROPOSITION = "annexe_2026_croisement"
...
def charger_catalogue_pistes(chemin: Path | None = None) -> dict[str, Any]:
    ...
    if not isinstance(data.get("pistes"), list):
        raise ValueError("catalogue pistes Annexe invalide : clé pistes absente")
    return data
...
def _charge_utile(piste: dict[str, Any], catalogue: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "pages": piste.get("pages") or [],
        "extrait_annexe": piste.get("extrait_annexe"),
    ...
    return [
        {
            ...
            "pages": piste.get("pages") or [],
            "extrait": piste.get("extrait_annexe"),
            "rapport": catalogue.get("rapport"),
        }
    ]
...
def lier_propositions_ouvertes(
    ...
    """Map entree_id → méta piste + proposition_id ouverte (si seedée)."""
    cat = catalogue or charger_catalogue_pistes()
    ...
    for eid, piste in index.items():
        ...
        # Annexe : souvent suggestion textuelle seule — patch aperçu possible,
        # appliquer seulement si champ/retrait structurés.
        out[eid] = {
            "piste_annexe": True,
            "lot": cat.get("lot"),
            "piste_id": piste["piste_id"],
            ...
            "peut_appliquer": bool(prop_id and (champ or retirer_ok)),
            "statut_editorial": piste.get("statut_editorial") or "a_valider_humain",
...
def enrichir_entrees_file(
    ...
    """Ajoute badge / lien proposition sur les entrées de la file a_confirmer."""
    ...
    for e in entrees:
        ...
        meta = liens.get(eid)
        if meta:
            copie["piste_annexe"] = True
            copie["piste_annexe_meta"] = meta
            copie["proposition_id"] = meta.get("proposition_id")
            copie["proposition_id_annexe"] = meta.get("proposition_id")
            copie["peut_preparer_patch"] = bool(meta.get("peut_preparer_patch"))
            copie["peut_appliquer"] = bool(meta.get("peut_appliquer"))
            copie["retirer_a_confirmer_autorise"] = bool(
                meta.get("retirer_a_confirmer_autorise")
            )
        else:
            copie["piste_annexe"] = False
