...
def contexte_tenant(session: Session, tenant_id: int) -> Iterator[None]:
    ...
    # set_config(..., is_local=true) ≡ SET LOCAL : portee = transaction.
    # On ne peut pas ecrire SET LOCAL ... = :t — PostgreSQL refuse les
    # parametres lies sur SET. Jamais set_config(..., false) ni SET nu.
    session.execute(
...
def effacer_contexte_tenant(session: Session) -> None:
    """Retire le GUC pour le reste de la transaction (NULL → refus par defaut)."""
